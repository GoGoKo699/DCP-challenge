...
They have intentionally not been edited to hide or rewrite the historical analysis.
...
- `verification.py`, `benchmarking.py`, and `IBM.py` use the legacy fluctuation formula rather than the Bernoulli standard error.
- `IBM.py` uses prepared `x0,x1` values as a collision condition, which is not the measured Fourier-label event described by the protocol.

Corrected implementations are under `src/dcp_challenge/`. The modern tests do not import the original scripts.
