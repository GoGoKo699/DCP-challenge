...
def DCP_samples(n,s,error):
    ...
    for i in range(1,n+1):
        if s_bin[i-1]==x_bin[i-1]:
           ...
           if s_bin[i-1]==0:
              ...
              c.add(gates.CNOT(0,i))
    '''
    Bob: apply QFT on n bits then mearue it
    '''
    c.add(QFT(n).on_qubits(*range(1,n+1)))
    if error==True:
    ...
    if error==True:
       ...
       a=a+(1-2*a)*flip
    '''
    return measurement as a list and the state of the reflection qubit
    '''
    return a,state
...
def DCP_samples_Hbasis(n,s):
    ...
    for i in range(0,n+1):
        c.add(gates.H(i))
    output=c.add(gates.M(*range(0,n+1),collapse=True))
    result = c()
    a = output.samples()[0]
    state = output.samples(binary=False)[0]
    return state

def DCP_Hbasis(s,n,m,t):
    i=0
    while i<m*t:
          result=DCP_samples_Hbasis(n,s)
          if result==1:
             return 0
          if result==2**n+1:
             return 1
