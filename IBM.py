...
plt.hist(bit_data,density=True,bins=np.arange(0.5, 1, 0.5/250),align='mid',color='deepskyblue',alpha = 0.85,label=''r'$\mathbf{p}_{clean}$')
plt.hist(IBM_data,density=True,bins=np.arange(0.5, 1, 0.5/250),align='mid',color='hotpink',alpha = 0.85,label=''r'$\mathbf{p}_{IBM}$')
plt.plot(x, stats.norm.pdf(x, mean_bit, sigma_bit), color='steelblue')
plt.plot(x, stats.norm.pdf(x, mean_IBM, sigma_IBM), color='mediumvioletred')
plt.xlim(0.495,1.05)
plt.ylim(0,63)
plt.legend()
