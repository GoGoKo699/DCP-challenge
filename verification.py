...
def main(m, n, t):
    ...
    plt.hist(distribution_Hbasis,density=True,bins=np.arange(0.5, 1, 0.5/250),align='mid',color='limegreen',label=''r'$\mathbf{p}_{Hbasis}$')
    plt.axvline(x=p_upper,color='k',linestyle='dashed',label=''r'$p_{upper}$') 
    plt.axvline(x=p_B,color='gray',linestyle='dashed',label=''r'$p_{B}$')
    plt.plot(x, stats.norm.pdf(x, p_clean, sigma_clean), color='steelblue')
    plt.plot(x, stats.norm.pdf(x, p_Hbasis, sigma_Hbasis), color='green')
    plt.xlim(0.495,1.05)
    plt.ylim(0,63)
    plt.xlabel('Accuracy')
