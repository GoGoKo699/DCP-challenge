...
def main(m, n, t):
    ...
    '''
    p_birthday=proba.k_upper(n,m)
    t_estimate=(np.log(5)-np.log(2))/(np.log(2)-np.log(1+p_birthday))
    ...
    plt.hist(distribution,density=True,bins=np.arange(0.5, 1, 0.5/500),align='mid',color='hotpink',label=''r'$\mathbf{p}_{bit}$')
    ...
    plt.axvline(x=p_error,color='b',linestyle='dashed',label=''r'$\mathbf{p}_{error}$')
    plt.axvline(x=p_clean,color='m',linestyle='dashed',label=''r'$\mathbf{p}_{clean}$')
    plt.xlim(0.495,1.05)
    plt.legend()
    plt.title('Probability distribution m=%i'%m+',n=%i'%n+',t=%i'%t)
    plt.xlabel('Accuracy')
    plt.ylabel('Probability distribution')
    plt.tight_layout()
    plt.savefig('Probability distribution m=%i'%m+',n=%i'%n+',t=%i'%t+'.pdf')
    plt.show()
